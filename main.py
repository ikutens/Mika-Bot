...
async def get_spotify_album_url(token, album, artist):
    """
    Returns (spotify_url, cover_url, tracklist)
    If not found, returns (None, None, None)
    This version grabs more than 1 result, and picks the closest album by both album name and artist.
    """
...
async def resolve_discogs_artist_name(user_input, discogs_token):
    """
    Fast, robust artist name resolver for Discogs.
    - Checks top 5 results from Discogs search.
    - Fuzzy-matches user input to titles.
    - Returns best match if score >= 75, otherwise just returns the user input.
    """
    ...
    async with session.get(url) as resp:
        ...
        best_match, score, idx = process.extractOne(user_input, titles, scorer=fuzz.ratio)
...
# ======= Triggers & Helper Messaging =======
media_triggers = [
    (["wow", "וואו"], "wow.jpg"),
    (["מכשפה בוכרית"], "yerk.png"),
    (["noob", "נוב"], "the_noob.mp4"),
    (["you can call me deku","gosling","גוסלינג"], "deku.mp4"),
    (["you can call me miku"], "miku-final.png"),
    (["fyp", "for you", "פור יו", "tiktok", "טיקטוק"], "fyp.mp4"),
    (["warden", "וורדן"], "./Fish/Warden.png")
]
def suicide_triggers(msg):
...
async def on_message(message):
    ...
    # --- Reset streak if a command is used ---
    ...
    # Custom triggers
    if word_in_text("מה אומר דוד", msg):
    ...
    for user in message.mentions:
        if user.id == 168329788325363712 and not message.reference:
            await message.reply("אה סנקי")
            return
        if user.id == 201051167084642304 and not message.reference:
            await message.reply("https://hebrew-academy.org.il/keyword/%D7%91%D6%BC%D6%B9%D7%94%D6%B6%D7%9F/")
            return
        if user.id == 343667951959932940 and not message.reference:
...
async def update_reviews(ctx):
    if ctx.author.id != 338054995209355274:
...
@bot.command()
async def album(ctx, *, filters: str = ""):
